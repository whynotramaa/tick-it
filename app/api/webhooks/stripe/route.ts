...
export async function POST(req: Request) {
    ...
    if (event.type === "checkout.session.completed") {
        ...
        const metadata = session.metadata as StripeCheckoutMetaData; // Assuming `StripeCheckoutMetaData` is a defined type
        console.log("Session metadata:", metadata);
        console.log("Convex client:", convex); // Assuming `convex` is an initialized Convex client
